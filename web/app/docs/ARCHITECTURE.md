...
- Per-election mapped, pending, and unmapped vote coverage.
...
Ballot letters are not stable party identities across elections. Version 1 therefore generates deterministic placeholder colors by election plus ballot letter and marks them `provisional`. Reviewed names and colors can be added to `config/party-overrides.json` without touching the compiler or UI.
...
Current simplified source sizes are about 11 MB for statistical areas and 7 MB for localities. Version 1 uses pruned GeoJSON URLs because it keeps the pipeline simple and is adequate for local development.
...
- Desktop uses floating side panels; mobile uses a full-width top control group and bottom result sheet.
...
1. Complete/review the production geography assignments and rerun the compiler.
...
- No editing of the Python pipeline or active geocoding work.
